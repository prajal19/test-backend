...
//     // Get all active morning carpools for today
...
//     // Check if user already has an active morning carpool for this date
...
//         message: `A new morning carpool has been created for ${new Date(date).toLocaleDateString()}`,
...
export async function POST(request) {
  try {
    ...
    // Check if driver exists and is available for morning
    const driver = await Parent.findById(driverId);
    if (!driver || !driver.isAvailable.morning) {
      return NextResponse.json({ error: 'Driver not available for morning carpool' }, { status: 400 });
    }
    
    // Check if driver has already created a morning carpool for this date
    const existingCarpool = await Carpool.findOne({
      driver: driverId,
      type: 'morning',
      date: new Date(date)
    ...
    if (existingCarpool) {
      return NextResponse.json({ error: 'Driver already has a morning carpool for this date' }, { status: 400 });
    }
    ...
    return NextResponse.json({
      message: 'Morning carpool created successfully',
      carpool
    }, { status: 201 });
  } catch (error) {
    return NextResponse.json({ error: error.message }, { status: 500 });
